...
public class agregaraccidente extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String detalle1 = request.getParameter("detalle1");
            String q = "INSERT INTO accidentes VALUES ('" + accidente + "','" + detalle1 + "')";
            connSqlServer con = new connSqlServer();
            try {
                con.querySql(q);
                response.sendRedirect("admon.jsp");
            } catch (SQLException ex) {
                Logger.getLogger(agregaraccidente.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
